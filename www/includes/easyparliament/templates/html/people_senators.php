...
if ($order == 'party') {
	$th_party = 'Party';
} elseif ($order == 'electorate') {
	$th_electorate = 'Electorate';
}
...
function render_mps_row($mp, &$style, $order, $MPURL) {
	...
	$name = member_full_name(1, $mp['title'], $mp['first_name'], $mp['last_name'], $mp['electorate']);
...
#	$MPURL->insert(array('pid'=>$mp['person_id']));
	?>
				...
				<td class="row-<?php echo $style; ?>"><a href="<?php echo $MPURL->generate().make_member_url($mp['first_name'].' '.$mp['last_name'], $mp['electorate'], 1); ?>"><?php echo $name; ?></a></td>
				<td class="row-<?php echo $style; ?>"><?php echo $mp['party']; ?></td>
				<td class="row-<?php echo $style; ?>"><?php echo $mp['electorate']; ?></td>
